...
export const ContentPageLabelOverview: FunctionComponent<
	DefaultComponentProps
> = ({ className }) => {
	// Hooks
	const history = AdminConfigManager.getConfig().services.router.useHistory();
	const [contentPageLabels, setContentPageLabels] = useState<
		ContentPageLabel[] | null
	>(null);
	const [contentPageLabelCount, setContentPageLabelCount] = useState<number>(0);
	const [contentPageLabelIdToDelete, setContentPageLabelIdToDelete] = useState<
		number | null
	>(null);
	...
	const [tableState, setTableState] = useState<
		Partial<ContentPageLabelTableState>
	>({});
	...
	const generateWhereObject = useCallback(
		(filters: Partial<ContentPageLabelTableState>) => {
			// biome-ignore lint/suspicious/noExplicitAny: todo
			...
			andFilters.push(
				...getMultiOptionFilters(filters, ["content_type", "language"]),
			);
			...
			const [contentPageLabelTemp, contentPageLabelCountTemp] =
				await ContentPageLabelService.fetchContentPageLabels(
					...
					(tableState.sort_column ||
						"updated_at") as ContentPageLabelOverviewTableCols,
	...
	const contentTypeOptions = contentTypes.map(
		(option): CheckboxOption => ({
			...
			label: option.label,
			checked: (
				get(tableState, "content_type", [] as string[]) as string[]
	...
	const handleDelete = async () => {
		...
		await ContentPageLabelService.deleteContentPageLabel(
			contentPageLabelIdToDelete,
		);
			...
			<>
				<FilterTable
					...
					renderCell={(rowData: ContentPageLabel, columnId: string) =>
						renderTableCell(
							rowData,
							columnId as ContentPageLabelOverviewTableCols,
						)
					}
			...
			<AdminLayout.Actions>
				<Button
					...
					onClick={() =>
						history.push(
							AdminConfigManager.getAdminRoute(
								"ADMIN_CONTENT_PAGE_LABEL_CREATE",
							),
						)
					}
					variants={["primary"]}
