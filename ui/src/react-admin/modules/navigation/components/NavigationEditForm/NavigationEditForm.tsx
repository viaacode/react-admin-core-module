...
export const NavigationEditForm: FunctionComponent<NavigationEditFormProps> = ({
	...
	const [permissionWarning, setPermissionWarning] = useState<ReactNode | null>(
		null,
	);

	const languageOptions = (allLanguages || []).map(
		(languageInfo): SelectOption => {
			return {
	...
	const checkMenuItemContentPagePermissionsMismatch = useCallback(
		(contentUserGroupIds: (string | number)[]) => {
			...
			const navItemUserGroupIds: string[] =
				currentNavigationItem.userGroupIds || [];
			const allUserGroupIds: string[] = allUserGroups.map((ug) =>
				String(ug.value),
			);
			...
			const faultyUserGroupIds = without(
				navItemUserGroupIds,
				...contentUserGroupIds,
			);
	...
	return (
		<Form className="m-menu-edit-form">
			...
			>
				...
				<CreatableSelect
					value={navigationParentOptions.find(
						(opt) => opt.value === navigationParentId,
					)}
			...
			{enableIcons && (
				<FormGroup
					label={tText(
						"admin/menu/components/menu-edit-form/menu-edit-form___icoon",
					)}
				>
					<IconPicker
			...
			<FormGroup
				...
				label={tText(
					"admin/menu/components/menu-edit-form/menu-edit-form___label",
				)}
			>
				<TextInput
					onChange={(value: string) =>
			...
			<FormGroup
				...
				label={tText(
					"admin/menu/components/menu-edit-form/menu-edit-form___tooltip",
				)}
			>
				<TextInput
					onChange={(value: string) =>
			...
			<FormGroup
				...
				label={tText(
					"admin/menu/components/menu-edit-form/menu-edit-form___link",
				)}
				required
			>
				<ContentPicker
					...
					value={
						currentNavigationItem.contentType &&
						currentNavigationItem.contentPath
							? {
					...
					<ReactSelect
						...
						value={
							languageOptions.find(
								(option) =>
									option.value ===
									(currentNavigationItem.language || Locale.Nl),
							) || languageOptions[0]
						}
						onChange={(evt) =>
							onChange({
			...
			<Grid>
				...
				<Column size="2-7" className="c-navigation-edit-form__warnings">
					...
					{permissionWarning && (
						<Alert message={permissionWarning} type="danger" />
					)}
				</Column>
			</Grid>
		</Form>
