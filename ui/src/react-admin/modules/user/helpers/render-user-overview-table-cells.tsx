	...
	switch (columnId) {
		...
		case "tempAccess": {
			if (hasTempAccess(tableRowCommonUser?.tempAccess)) {
				...
				return tHtml(
					"admin/users/views/user-overview___tijdelijke-toegang-nee",
				);
			}
		}
		case "tempAccessFrom":
		...
		case "educationLevels": {
			...
			return (
				stringsToTagList(labels, null, info.navigateFilterToOption(columnId)) ||
				"-"
			);
		}
		...
		case "userGroup":
			return truncateTableValue(
				tableRowCommonUser.userGroup?.label ||
					tableRowCommonUser.userGroup?.name ||
					"-",
			);
		...
		case "actions":
			return (
				<ActionsDropdown
					...
					onOptionClicked={() =>
						handleOptionClicked(tableRowCommonUser.profileId)
					}
				/>
			);
...
export function renderUserOverviewTableCellText(
	...
	switch (columnId) {
		...
		case "idps":
			return (
				(Object.keys(tableRowCommonUser?.idps || {}) as Idp[]).join(", ") || ""
			);
		...
		case "educationalOrganisations": {
			...
			return orgs
				.map(
					(org): string =>
						org.organisationLabel || org.unitId || org.organisationId,
				)
				.join(", ");
		}
