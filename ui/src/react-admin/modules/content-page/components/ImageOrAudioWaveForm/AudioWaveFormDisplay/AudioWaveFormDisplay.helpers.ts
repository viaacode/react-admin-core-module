...
// Bar geometry traced from Frame 1895.svg: 30 vertical bars, 3 units apart starting at x=0.9,
// centered on y=21.6, in a 92x44 viewBox.
...
// Padding around the bars, as a fraction of the display's box on each side. Baked into the
// viewBox rather than CSS padding so it can't collapse to zero on a short/narrow container --
// `preserveAspectRatio="xMidYMid meet"` on the <svg> then scales and centers it without stretching.
const WAVE_FORM_PADDING_X_RATIO = 0.15;
const WAVE_FORM_PADDING_Y_RATIO = 0.3;

// Half the height of each bar (viewBox units), left to right, traced from Frame 1895.svg.
...
// Large: the small wave form immediately followed by its own mirror, on one continuous grid --
// not two small viewBoxes side by side, which would double their outer margins into a bigger gap.
