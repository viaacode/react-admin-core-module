...
const ContentBlockFormGroup: FunctionComponent<ContentBlockFormGroupProps> = ({
	...
	<div className="c-content-block-form-group">
		{Object.keys(formGroup.fields).map(
			(key: string, formGroupIndex: number) => {
				let error: string[];
				const configErrors = config.errors || {};
				const stateKey = key as
					| keyof ContentBlockComponentState
					| keyof ContentBlockState;
				...
				return (
					...
					>
						<FormGroup
							label={field.repeat ? undefined : field.label}
							error={error}
						>
							<ContentBlockFields
