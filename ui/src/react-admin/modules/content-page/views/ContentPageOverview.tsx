...
export const ContentPageOverview: FunctionComponent<
	ContentPageOverviewProps
> = ({ commonUser }) => {
	// Hooks
	const [contentToDelete, setContentToDelete] =
		useState<ContentPageInfo | null>(null);
	const [isConfirmModalOpen, setIsConfirmModalOpen] = useState<boolean>(false);
	const [isNotAdminModalOpen, setIsNotAdminModalOpen] =
		useState<boolean>(false);
	const [tableState, setTableState] = useState<Partial<ContentTableState>>({});
	const [loadingInfo, setLoadingInfo] = useState<LoadingInfo>({
		state: "loading",
	});
	const [userGroupOptions, userGroups] = useUserGroupOptions(
		"CheckboxOption",
		false,
		true,
	) as [CheckboxOption[], UserGroupWithPermissions[], boolean];
	const [contentTypes] = useContentTypes();
	const [contentPageLabelOptions] = useContentPageLabelOptions();
	const { data: allLanguages } = useGetAllLanguages();
	const [languageOptions] = useGetLanguageFilterOptions();
	const [selectedDropdownContentPageId, setSelectedDropdownContentPageId] =
	...
	const contentTypeOptions = useMemo(() => {
		return contentTypes.map(
			(option): CheckboxOption => ({
				id: option.value,
				label: option.label,
				checked: (tableState?.contentType || ([] as string[])).includes(
					option.value,
				),
			}),
		);
	...
	const tableColumns = useMemo(() => {
		return GET_OVERVIEW_COLUMNS(
			...
			setSelectedCheckboxes(
				userGroupOptions,
				(tableState?.userGroup || []).map((userGroup) =>
					String(userGroup),
				) as string[],
			),
			setSelectedCheckboxes(
			...
			setSelectedCheckboxes(
				languageOptions,
				(tableState?.translations || []) as string[],
			),
		);
	}, [
		contentPageLabelOptions,
	...
	const generateWhereObject = (filters: Partial<ContentTableState>) => {
		...
		if (userGroupIdsFilter) {
			userGroupIdsFilter.profile.profile_user_group.group.id._in =
				userGroupIdsFilter.profile.profile_user_group.group.id._in.map(
					...
					<>
						...
						>
							<DropdownButton>
								<Button
									title={tText(
										"modules/content-page/views/content-page-overview___hover-opties",
									)}
									type="secondary"
