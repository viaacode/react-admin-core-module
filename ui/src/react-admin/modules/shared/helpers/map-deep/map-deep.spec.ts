...
import {
	mockObject1,
	mockObject2,
} from "~shared/helpers/map-deep/map-deep.mocks";
...
describe("map-deep", () => {
	it("Should correctly map all levels of an object", () => {
		...
		expect(resultObj.classes[0].birthDay).toEqual(
			new Date(mockObject1.classes[0].birthDay),
		);
		expect(resultObj.classes[1].birthDay).toEqual(
			new Date(mockObject1.classes[1].birthDay),
		);
		expect(resultObj.classes[2].birthDay).toEqual(
			new Date(mockObject1.classes[2].birthDay),
		);
