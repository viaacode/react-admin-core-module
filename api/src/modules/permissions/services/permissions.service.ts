...
export class PermissionsService {
	...
	public async getPermissions(): Promise<PermissionData[]> {
		const response = await this.dataService.execute<PermissionQueryTypes['GetPermissionsQuery']>(
		...
		if ((response as PermissionQueryTypes['GetPermissionsQueryAvo'])?.users_permissions) {
			return (response as PermissionQueryTypes['GetPermissionsQueryAvo'])?.users_permissions.map(
				(permission) => {
					return {
						id: permission.id,
