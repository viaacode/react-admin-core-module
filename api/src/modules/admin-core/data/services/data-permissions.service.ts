...
async function canUpdateContentBlocks(
	...
	const contentPage: Avo.ContentPage.Page =
		await this.contentPagesService.getContentPageByContentBlockId(
			contentBlockId,
		);
...
export class DataPermissionsService {
	...
	public async initWhitelist(): Promise<void> {
		this.whitelistEnabled = getConfig(
			this.configService,
			'graphQlEnableWhitelist',
		);
		...
		const whitelistFiles: Record<QueryOrigin, string[]> = {
			PROXY: [
				...
				'scripts/proxy-whitelist-hetarchief.json',
			],
		...
		Object.keys(whitelistFiles).forEach((whitelistKey) => {
			...
			whitelistPaths.forEach((whitelistPath) => {
				...
				const whitelistFileContent = fs.existsSync(absoluteWhitelistPath)
					? fs
							.readFileSync(absoluteWhitelistPath, { encoding: 'utf-8' })
							.toString()
					: '{}';
		...
		this.QUERY_PERMISSIONS = {
			ADMIN_CORE: {
				...
				GetPermissionsFromContentPageByPath: or(
		...
		const missingPermissionChecks = without(
			whitelistKeys,
			...queryPermissionKeys,
		);
	...
	public isQueryWhitelisted(queryDto: GraphQlQueryDto): boolean {
		...
		const queryName = this.getWhitelistedQueryName(
			queryDto.query,
