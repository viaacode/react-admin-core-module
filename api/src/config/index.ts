...
const config = (): Configuration => {
	...
	return {
		...
		GRAPHQL_ENABLE_WHITELIST:
			getEnvValue('GRAPHQL_ENABLE_WHITELIST', false) === 'true',
		...
		SAML_IDP_META_DATA_ENDPOINT: getEnvValue(
			'SAML_IDP_META_DATA_ENDPOINT',
			true,
		),
		SAML_SP_ENTITY_ID: getEnvValue('SAML_SP_ENTITY_ID', true),
		SAML_SP_PRIVATE_KEY: cleanMultilineEnv(
			getEnvValue('SAML_SP_PRIVATE_KEY', false),
		),
		SAML_SP_CERTIFICATE: cleanMultilineEnv(
			getEnvValue('SAML_SP_CERTIFICATE', false),
		),
		SAML_MEEMOO_IDP_META_DATA_ENDPOINT: getEnvValue(
			'SAML_MEEMOO_IDP_META_DATA_ENDPOINT',
			true,
		),
		SAML_MEEMOO_SP_ENTITY_ID: getEnvValue('SAML_MEEMOO_SP_ENTITY_ID', true),
		CORS_ENABLE_WHITELIST:
			getEnvValue('CORS_ENABLE_WHITELIST', false) === 'true',
		CORS_OPTIONS: {
			origin: (
				origin: string,
				...
				if (getEnvValue('CORS_ENABLE_WHITELIST', false) !== 'true') {
					...
					callback(
						new ForbiddenException('Request not allowed by CORS'),
						false,
					);
				}
			},
		...
		TICKET_SERVICE_CERTIFICATE: cleanMultilineEnv(
			getEnvValue('TICKET_SERVICE_CERT', true),
		),
		TICKET_SERVICE_KEY: cleanMultilineEnv(
			getEnvValue('TICKET_SERVICE_KEY', true),
		),
		TICKET_SERVICE_PASSPHRASE: getEnvValue('TICKET_SERVICE_PASSPHRASE', true),
		TICKET_SERVICE_MAX_AGE: parseInt(
			getEnvValue('TICKET_SERVICE_MAXAGE', true),
			10,
		),
		MEDIA_SERVICE_URL: getEnvValue('MEDIA_SERVICE_URL', true),
		ENABLE_SEND_EMAIL: getEnvValue('ENABLE_SEND_EMAIL', true) === 'true',
		CAMPAIGN_MONITOR_API_ENDPOINT: getEnvValue(
		...
		MULTER_OPTIONS: {
			...
			fileFilter: (req, file, cb) =>
				cb(null, VALID_MIME_TYPES.includes(file.mimetype)),
		},
		MEEMOO_ADMIN_ORGANIZATION_IDS: (
			getEnvValue('MEEMOO_ADMIN_ORGANIZATION_IDS', true) || ''
		)
			.split(',')
			.map((orgId) => orgId.trim()),
		REROUTE_EMAILS_TO: getEnvValue('REROUTE_EMAILS_TO', false),
		IGNORE_OBJECT_LICENSES:
			getEnvValue('IGNORE_OBJECT_LICENSES', false) === 'true',
		ORGANIZATIONS_API_V_2_URL: getEnvValue('ORGANIZATIONS_API_V2_URL', true),
		ELASTICSEARCH_LOG_QUERIES:
			getEnvValue('ELASTICSEARCH_LOG_QUERIES', false) === 'true',
		GRAPHQL_LOG_QUERIES: getEnvValue('GRAPHQL_LOG_QUERIES', false) === 'true',
		CLIENT_API_KEY: getEnvValue('CLIENT_API_KEY', true),
		IS_ADMIN_CORE_DEMO_APP:
			getEnvValue('IS_ADMIN_CORE_DEMO_APP', false) === 'true',
	};
};
